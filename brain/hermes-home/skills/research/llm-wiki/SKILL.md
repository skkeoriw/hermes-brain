...
2. Before every build, ingest, or query: verify a clean worktree, `git fetch`, `git checkout <branch>`, and `git pull --ff-only` from the canonical remote. Stop and report if there are uncommitted changes unless the user explicitly asked to preserve/commit them.
   ...
   - If `stash pop` reports conflicts, apply the three-case decision tree in step **8** below (Case A/B/C). Do not blindly abort — stale artifact conflicts (Case C) are non-blocking.
...
8. When the payload requires a detailed run log under `logs/webhook-runs/<run_id>.md`, include the run log in the same run's commit, and record start time, payload, sync command/result, key files read, created/updated/deleted lists, git status, errors, and end time.
   ...
   - If `git stash pop` fails, classify the failure using this three-case decision tree:
     - **Case A — Merge/content conflict on actual wiki/raw data files:** Abort the run. Log conflict details. Do NOT force-commit partial conflict state.
     - **Case B — Collision only on the current run-log path** (`logs/webhook-runs/<run_id>.md` already exists, "no checkout"): Treat as non-blocking warning. Log it. Continue processing. The run-log will be appended to the existing file.
     - **Case C — Conflict only on stale/old run-log files** (deleted by remote, modified in stash; e.g., `modify/delete` conflicts on `logs/webhook-runs/gh-<old>.md`): Non-blocking. Resolve with `git rm --cached <file> && rm -f <file> && git stash drop`. Log the resolution and continue. These are artifacts from previous failed-push runs — they are safe to discard because origin has no record of them.
   - **Divergence recovery + stash pop interaction:** If `git pull --ff-only` failed and you ran `git reset --hard origin/main` (divergence remediation), the subsequent `git stash pop` has elevated conflict risk because the worktree was force-replaced, not fast-forwarded. Expect more orphan/stale file conflicts (especially old run-logs and untracked artifacts). Apply Case A/B/C rules accordingly — stale artifact conflicts (Case C) are particularly common in this path.
