---
name: notebooklm-cli
description: NotebookLM CLI and API with live-session auth recovery. Full programmatic access to NotebookLM including features not in the web UI. Create notebooks, add sources, generate all artifact types, and download in multiple formats. Activates on explicit /notebooklm-cli or intent like "create a podcast about X"
---

# NotebookLM CLI

Complete programmatic access to Google NotebookLM—including capabilities not exposed in the web UI. Create notebooks, add sources (URLs, YouTube, PDFs, audio, video, images), chat with content, generate all artifact types, and download results in multiple formats. The installed CLI binary is still `notebooklm`.
...
- `npx skills add ChangfengHU/notebooklm-cli` installs this skill from the GitHub repository into compatible agent skill directories.
...
**Explicit:** User says "/notebooklm-cli", "/notebooklm", "use notebooklm", or mentions the tool by name
...
### Cross-machine auth (Mac browser -> Linux CLI)
Reference: `references/cross-machine-cookie-auth.md` for failure signatures and a validated recovery sequence.

When a user is logged in on another machine (e.g., Mac) and wants Linux to reuse that session:

1. Prefer exporting a Playwright `storage_state.json` on the source machine (not Netscape cookies.txt).
2. Validate on source machine first:
   - `notebooklm --storage ~/storage_state.json auth check --test`
3. Copy to Linux profile path used by CLI:
...
Pitfalls:
- Some builds may appear to ignore `--storage` for `auth check` subcommands; if so, place the file in the active profile path directly (`~/.notebooklm/profiles/<profile>/storage_state.json`) and re-test.
- `notebooklm login --browser-cookies chrome|chromium|edge` can fail with "can't find cookies file" in VNC/container/custom-profile setups because rookiepy only scans standard browser paths.
- Having SID/OSID cookies is not sufficient; token fetch can still fail if session is device-bound/expired. Always trust `auth check --test` over static cookie presence.
- Never ask users to paste raw cookies in chat/logs. Treat cookie dumps and storage_state as live credentials.
