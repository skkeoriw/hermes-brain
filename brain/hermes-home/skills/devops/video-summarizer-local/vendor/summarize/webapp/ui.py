...
from webapp.i18n import get_text, get_language_selector, init_i18n
...
def _render_sidebar(providers, default_provider, defaults, prompt_types):
    ...
    with st.sidebar:
        ...
        selected_provider = st.selectbox(get_text("PROVIDER"), provider_names, index=default_idx)
        ...
        prompt_type = st.selectbox(get_text("STYLE"), prompt_types, index=prompt_idx)
        ...
        # -- YAML editor --
        with st.expander(get_text("EDIT_YAML")):
            ...
            with col_save:
                if st.button(get_text("SAVE"), use_container_width=True):
                    try:
            ...
            with col_reset:
                if st.button(get_text("RELOAD"), use_container_width=True):
                    st.rerun()

        # -- History --
        if st.session_state.history:
            st.divider()
            st.markdown(f"### {get_text('HISTORY')}")
...
def _render_url_tab(sidebar, defaults):
    ...
    with col2:
        url_btn = st.button(
            get_text("RUN"), type="primary", use_container_width=True, key="run_url"
        )

    if url_btn and video_url:
        if not video_url.startswith("http"):
            st.warning(get_text("ENTER_URL") if st.session_state.get("app_language") == "zh" else "URL must start with http or https")
            return
        status_ctx = st.status(get_text("PROCESSING"), expanded=False)
        with status_ctx:
            try:
                source_type = (
...
def _render_file_tab(sidebar, defaults):
    ...
    if uploaded_state:
        ...
        if st.button(get_text("CLEAR_FILE"), use_container_width=True, key="clear_uploaded_file"):
            clear_uploaded_file_state()
            st.rerun()

    if st.button(get_text("RUN"), type="primary", disabled=uploaded_state is None, key="run_file"):
...
def main():
    ...
    # -- Main area --
    title_text = "🎬 SUMMARIZE" if st.session_state.get("app_language") == "en" else "🎬 摘要生成器"
    st.markdown(f'<h1 class="main-header">{title_text}</h1>', unsafe_allow_html=True)

    tab_url_label = get_text("URL_TAB")
    tab_file_label = get_text("FILE_TAB")
    tab_url, tab_file = st.tabs([tab_url_label, tab_file_label])
    with tab_url:
