...
def _transcribe_cloud_whisper(
    ...
    # Try providers in order: Groq → OpenAI → OpenRouter
    ...
    if not api_key:
        raise APIKeyError("No Whisper API key found. Set one of: GROQ_API_KEY, OPENAI_API_KEY, or OPENROUTER_API_KEY in .env")

    spinner = ProgressSpinner(f"Transcribing audio with {provider.upper()} API", verbose)
    try:
        spinner.start()
        ...
        if provider == "groq":
            ...
            with open(audio_path, "rb") as audio_file:
                ...
                spinner.stop()
                print_status("Audio transcription completed via Groq", "SUCCESS", verbose)
                return transcript
            ...
            with open(audio_path, "rb") as audio_file:
                ...
                # Only add language if specified (OpenAI expects language code like 'zh', 'en')
                if language and language != "auto":
                ...
                transcript = f"{timestamp} {transcript_text.strip()}\n"

                spinner.stop()
                print_status("Audio transcription completed via OpenAI", "SUCCESS", verbose)
                return transcript
            ...
            try:
                ...
                spinner.stop()
                raise TranscriptError(
                    "OpenRouter requires 'openai' package: pip install openai"
                )
            ...
            with open(audio_path, "rb") as audio_file:
                ...
                spinner.stop()
                print_status("Audio transcription completed via OpenRouter", "SUCCESS", verbose)
                return transcript
