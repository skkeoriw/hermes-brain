...
def get_api_key(cfg: Dict) -> str:
    ...
    for pattern, env_var in API_PROVIDERS.items():
        if pattern in base_url:
            ...
            # Try both lowercase and uppercase with _API_KEY suffix
            matched_key = os.getenv(env_var) or os.getenv(env_var.upper()) or os.getenv(f"{env_var.upper()}_API_KEY")
            if matched_key:
                break
    ...
    if not matched_key:
        ...
        if domain_match:
            ...
            matched_key = os.getenv(domain_name) or os.getenv(domain_name.upper()) or os.getenv(f"{domain_name.upper()}_API_KEY")
            matched_provider = domain_name

    # Try generic api_key as last resort
    if not matched_key:
        matched_key = os.getenv("api_key") or os.getenv("API_KEY")
