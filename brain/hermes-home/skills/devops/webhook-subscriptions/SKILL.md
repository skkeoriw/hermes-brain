...
For diagnosing and fixing git stash race conditions when multiple webhooks run concurrently (e.g. `could not restore untracked files from stash` failures), see `references/git-stash-race-conditions-in-concurrent-webhooks.md`. **Option 4 (`.gitignore` + GitHub Actions `concurrency` control) is production-proven as the most robust long-term solution** — combines root-cause elimination with a safety mechanism, and includes end-to-end tracing via standardized `run_id` format.
...
12. **If the webhook test returns `{"status":"accepted"}` but no agent logs appear, verify that the subscription's `provider` and `model` fields resolve to a valid model ID for that provider. An invalid model causes the agent to exit before tool execution, leaving only the webhook acceptance. Check gateway logs for `Non-retryable error (HTTP 400): ... is not a valid model ID`. Fix by updating the subscription with `hermes webhook update <name> --provider <valid> --model <valid>` or editing `~/.hermes/webhook_subscriptions.json` and restarting gateway.
12. **Webhook stash pop failures on concurrent execution?** You are experiencing a git race condition where untracked files (log files, build artifacts) created by concurrent webhooks block `git stash pop`. Implement Option 4 from `references/git-stash-race-conditions-in-concurrent-webhooks.md`: (1) Add log paths to `.gitignore` (root cause), (2) Add GitHub Actions `concurrency` control (safety mechanism). This dual-layer fix is production-proven and includes end-to-end tracing via `gh-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}` run_id format.

13. **GitHub push says triggered, but Hermes sees nothing?** Distinguish source failures vs receiver failures:
...
16. **Pin webhook delivery to a dedicated Telegram target (avoid home-channel drift):** For automation routes, set route-level delivery explicitly (`deliver: telegram` + `deliver_extra.chat_id`) instead of relying on default/home delivery. Verify with `hermes webhook list` that `Deliver: telegram` is shown for the route and keep the target chat isolated per workflow.
17. **Business summary must be quantitative, not generic:** For llm-wiki incremental notifications, require concrete deltas in the outgoing message: (a) raw file count + list, (b) new entity count + names, (c) new source count + names, (d) relationship delta counts by type + at least 3 relation examples, (e) impacted concept/overview/index pages, (f) commit hashes + push result + run-log path. If a metric cannot be computed, report `unknown` with reason.
18. **Audit `skipped:no_raw_changes` notification behavior with evidence (don’t infer):**
   - Check per-run file log under `logs/webhook-runs/<run_id>.md` for `decision: skipped:no_raw_changes` and `commit/push: none`.
   - Cross-check `~/.hermes/logs/gateway.log` for that same delivery id (`gh-...`) to see whether it only produced webhook callback lines (`Sending response ... to webhook:wiki-ops:...`) versus any explicit Telegram send tool result.
   - For definitive proof, check the webhook session file under `~/.hermes/sessions/session_<timestamp>_*.json` (where platform is `webhook` and matches the delivery time) for a `send_message` tool call with `\"success\": true`. If no such call exists, Telegram was not sent by the webhook logic.
   - If the user reports a Telegram message anyway, rule out operator-injected/manual test sends first (message can come from `send_message` outside webhook run and be mistaken as automation output).
19. **`send_message` executed but user cannot see message:** Distinguish transport from visibility. Verify active bot identity with `getMe` on current `TELEGRAM_BOT_TOKEN`, then verify delivery via direct Bot API `sendMessage` to the intended `chat_id`. If API returns `ok:true` with `message_id` but user still sees nothing, check account/chat mismatch (wrong Telegram account, archived chat, different chat_id) rather than webhook logic.
