...
3. 保护性同步：
   ...
   ```
4. 计算 raw 变更（优先用 payload 的 before/sha，fallback 用 HEAD~1..HEAD）：
   ```bash
   git diff --name-only {before}..{sha} -- 'raw/*.md' 'raw/**/*.md'
   ```
5. 过滤出 `raw/youtube-links/` 下变更的文件。
6. 若无变更：写日志 `skipped:no_raw_changes`，停止执行。

### 处理
7. 从变更文件中提取 YouTube URL（正则：`https://youtu(\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\.)?be(\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\.com)?/[\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\w\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\-?=&]+`）。
8. 确保目标目录存在：`mkdir -p {wiki_local_path}/raw/notebooklm-analysis {wiki_local_path}/raw/notebooklm-mindmaps`
9. 调用 NotebookLM processor（**必须用这个路径**）：
   ```bash
...
10. 将生成文件复制到仓库，**并按以下规则重命名为中文语义文件名**：
...
11. **记录结束时间并计算耗时**：
    ...
    ```
12. 确保日志目录存在：`mkdir -p {wiki_local_path}/logs/webhook-runs`
13. 写执行日志到 `{wiki_local_path}/logs/webhook-runs/{run_id}.md`，**必须包含**：
...
14. ```bash
    ...
    ```
15. 验证 push 成功（`git status` 确认与 origin/main 一致）。
